...
public class Kiwi extends Fruit {
    ...
    public Kiwi(String name, String size, String brand, double mass, String packet, int price, int quantity){ 			
       ...
       if(this.quantity == 1) {
      		...
      		System.out.printf("Total Price(packet) \t= RM%.2f", total(16.50));
      	}else if(this.quantity == 36) {
      		DiscountRate kp = new KiwiDiscount();
      		double kdis = kp.discountRate();
      		System.out.printf("Total Price(box) \t= RM%.2f", total(16,kdis));
      	}
     }
    //Overriding method
    ...
    public double total(double kprice2){
	    return this.quantity*kprice2;
   }
    
    //Overloading (2 parameters)
    public double total(double kprice3, double kdis){
   	    return (kprice3*this.quantity)*(1-kdis);
   }
